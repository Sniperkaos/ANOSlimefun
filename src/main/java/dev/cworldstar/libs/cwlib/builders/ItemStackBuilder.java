...
public class ItemStackBuilder {
	...
	public static final NamespacedKey COLOR_LERP_KEY = AbstractSFAddon.key("COLOR_LERP");
	...
	public ItemStackBuilder setNameLerp(String toLerp) {
		ItemMeta meta = item.getItemMeta();
		meta.getPersistentDataContainer().set(COLOR_LERP_KEY, PersistentDataType.STRING, toLerp);
	...
	public ItemStackBuilder attribute(String key, Attribute attribute, double value, EquipmentSlotGroup group, Operation type) {
		item.editMeta(meta -> {
			meta.addAttributeModifier(attribute, new AttributeModifier(ANOSF.key(key), value, type, group));
		});
		return this;
	}
	...
	public SlimefunItemStack asSlimefunItemStack(String id) {
		return new SlimefunItemStack(id, this.item());
	}

	public ItemStackBuilder desc(String[] strings) {
		return lore(strings);
	}
